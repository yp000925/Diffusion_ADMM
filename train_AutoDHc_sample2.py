...
model_name = "AutoDHc/"
timestr = time.strftime("sample2_%Y-%m-%d-%H_%M_%S/", time.localtime())
out_dir = 'output/'
...
img = plt.imread('PhasenNetSample.tiff')
...
holo = img[280:280+nx, 225:225+ny]
holo = holo / np.max(holo)
holo = torch.from_numpy(np.array(holo))
...
optimizer = Adam([o_phase], lr=0.001)
