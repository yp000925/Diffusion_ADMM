...
use the pre-trained model that is from DH data -> DnCNN_v2 learn the residual but output the estimated signal (input-residual)
"""
# from pnp_admm_1c import pnp_ADMM_DH
from pnp_admm_Unet1c_V0 import pnp_ADMM_DH
from utils import *
...
import os
from utils.load_model import load_model_DHTrained
...
pretrained_ckpt = "DuDNNDH.pth"
model = load_model_DHTrained(pretrained_ckpt,device)
...
opts = dict(rho=torch.tensor([5e-4]), maxitr=50, verbose=True, gt=torch.tensor(gt_intensity), eta=0.9,
