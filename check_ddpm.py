...
import torch
import numpy as np
from utils.functions import psnr, generate_otf_torch, zero_padding_torch, crop_img_torch,norm_tensor,display_sample,gray_to_rgb,rgb_to_gray_tensor
    ...
    def reconstruction(self, holo, gamma, visual_check=None):
        ...
        for i, t in enumerate(tqdm.tqdm(self.scheduler.timesteps)):
            ...
            with torch.no_grad():
                ...
                out = scheduler.step(residual, t, sample)
            ...
            if visual_check and (i + 1) % visual_check == 0:
                ...
                display_sample(norm_tensor(pred_x0),i+2)
        return sample
...
img = Image.open('ExpSample/celeA/testsample.jpeg').resize([256, 256])
...
out = diffuser.reconstruction(test, gamma=0.05,visual_check=10)
