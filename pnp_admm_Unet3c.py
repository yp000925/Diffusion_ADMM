...
timestr = time.strftime("Unet3c%Y-%m-%d-%H_%M_%S",time.localtime())
out_dir = 'output/'
...
class pnp_ADMM_DH():
    ...
    def denoise_step(self, o, u, denoiser, v_old):
        ...
        v_next = denoiser(v_tilde.unsqueeze(0)).to(torch.float32).to(self.device)
        v_next = v_next[0,0,:,:]
        v_next = rgb_to_gray(v_next.squeeze(0).cpu()).to(self.device).squeeze(0)
    ...
    def pnp_Admm_DH(self, y, opts):
        ...
        for i in pbar:
            ...
            o_psnr = psnr(o,gt)
            v_psnr = psnr(v,gt)
            # psnr_list.append(_psnr.cpu().numpy())
            early_stop = early_stop_checker(o_psnr.cpu().numpy())
